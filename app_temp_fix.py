# Temporary Fix for Missing approved_by Column
...
# Loan Model - WITHOUT approved_by column for now
class Loan(db.Model):
    __tablename__ = 'loans'
    id = db.Column(db.Integer, primary_key=True)
...
<body>
    <div class="login-container">
        <div class="logo">
            ...
            <p>Temporary Fix <span class="python-indicator">PYTHON</span> <span class="auto-refresh">WAITRESS</span></p>
        </div>
...
def dashboard():
    ...
    try:
        ...
        # Get pending loans for current user's stage only
        pending_loans_list = []
        if user.role in ['loan_officer', 'loan_manager', 'general_director', 'managing_director']:
            pending_loans_list = Loan.query.filter_by(
                current_stage=user.role,
                status='pending'
            ).all()
        
        portfolio_amount = f"${total_loan_amount:,.0f}"
...
<body>
    <div class="container">
        <div class="header">
            <div>
                <h1>🏦 Microfinance Platform <span class="python-indicator">PYTHON</span> <span class="auto-refresh">WAITRESS</span></h1>
        ...
        <div class="content">
            ...
            {"<div class='section'><h3>Pending Loans for Your Review</h3>" if pending_loans_list else ""}
            {"".join([f"<div class='loan-item'><h4>{loan.client.full_name}</h4><p>Amount: ${loan.amount:,.0f} • Purpose: {loan.purpose}</p><p>Applied: {loan.created_at.strftime('%Y-%m-%d')} • Client Income: ${loan.client.monthly_income:,.0f}/month</p><div class='loan-actions'><form method='post' action='/approve_loan_temp' style='margin: 0;'><input type='hidden' name='loan_id' value='{loan.id}'><input type='text' name='reason' placeholder='Approval/Rejection Reason' required style='width: 100%; margin-bottom: 5px; padding: 5px;' /><button type='submit' name='decision' value='approved' class='btn btn-approve'>Approve</button><button type='submit' name='decision' value='rejected' class='btn btn-reject'>Reject</button></form></div></div>" for loan in pending_loans_list]) if pending_loans_list else ""}
            {"</div>" if pending_loans_list else ""}
...
# Temporary approval route that doesn't use approved_by column
@app.route('/approve_loan_temp', methods=['POST'])
def approve_loan_temp():
    if 'user_id' not in session:
        return redirect(url_for('login'))
    ...
    try:
        ...
        flash('Loan approval error occurred')
        return redirect(url_for('dashboard'))
...
def apply_loan():
    ...
    try:
        ...
        flash('Loan application error occurred')
        return redirect(url_for('dashboard'))
...
if __name__ == '__main__':
    ...
    print("🚀 Starting Temporary Fix Microfinance Platform...")
    print("🌐 Access at: http://127.0.0.1:9000")
    print("✅ Features: Waitress Server, Temporary Fix (No approved_by column)")
